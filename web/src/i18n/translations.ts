...
export const translations = {
  en: {
    ...
    decisionListHint: 'Closes triggered by AI show "AI analysis close" in position history. If a cycle closes then reopens the same symbol, expand that cycle to see both actions.',
    systemExecutionHintCollapsed: 'System open/close this cycle, see System cycle',
    systemExecutionHintExpanded: 'This cycle was system open/close only. See System cycle chain.',
    systemCycleLabel: 'System cycles',
    sltpAnalysisCycleLabel: 'SL/TP analysis cycles',
    cyclesSessionTooltip: 'AI decision cycles this session',
    aiStrategyCol: 'AI strategy',
    slMultiplierCol: 'SL mult',
    trailIntensityCol: 'Trail',
    lockProfitPctCol: 'Lock %',
    cycleDeltaCol: 'Cycle Δ',
    advice_choppy_hold: 'Choppy hold',
    advice_trend_ride: 'Ride trend',
    advice_lock_profit: 'Lock profit',
    advice_trend_weakening: 'Trend weak',
    advice_high_vol_hold: 'High vol hold',
    trail_low: 'Low',
    trail_medium: 'Medium',
    trail_high: 'High',
    decisionCycleNote: 'Cycle numbers in the list are cumulative decision record IDs.',
    cycle: 'Cycle',
    success: 'Success',
    failed: 'Failed',
    ...
    historicalCyclesTooltip: 'Equity curve points (one recorded per cycle)',
    ...
    positionHistory: {
      ...
      totalTradesTooltip: 'Count of closed positions (one per symbol+side round-trip). Multiple add-on orders to the same position count as one trade.',
  ...
  zh: {
    ...
    decisionListHint: '由 AI 触发的平仓在持仓历史中显示「AI分析平仓」。若某周期对同一币种先平后开，请展开该周期查看完整操作序列。',
    systemExecutionHintCollapsed: '本周期为系统开/平仓，详见「系统周期」',
    systemExecutionHintExpanded: '本周期为系统开仓/平仓，详见「系统周期」思维链。',
    systemCycleLabel: '系统周期',
    sltpAnalysisCycleLabel: '止盈止损周期',
    cyclesSessionTooltip: '本会话 AI 决策周期数',
    aiStrategyCol: 'AI 策略',
    slMultiplierCol: 'SL 倍数',
    trailIntensityCol: '追踪强度',
    lockProfitPctCol: '锁利 %',
    cycleDeltaCol: '周期增量',
    advice_choppy_hold: '震荡持仓',
    advice_trend_ride: '抓住趋势',
    advice_lock_profit: '锁住利润',
    advice_trend_weakening: '趋势减弱',
    advice_high_vol_hold: '高波动持有',
    trail_low: '低',
    trail_medium: '中',
    trail_high: '高',
    decisionCycleNote: '列表中的周期号为决策记录序号（累计）。',
    cycle: '周期',
    success: '成功',
    failed: '失败',
    inputPrompt: '输入提示',
    ...
    historicalCyclesTooltip: '净值曲线点数（每周期记录一次）',
    displayRange: '显示范围',
    recent: '最近',
    allData: '全部数据',
    cycles: '个',
    ...
    positionHistory: {
      ...
      // Stats
      totalTrades: '总交易次数',
      totalTradesTooltip: '按平仓笔数统计：同一币种同方向从开到平算一笔；多次加仓会合并为一条持仓，平仓后只计 1 笔。若看到「3 次下单」但这里为 1，说明 3 次是加仓到同一持仓。',
